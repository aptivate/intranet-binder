...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if default_username:
            try:
                IntranetUser.objects.get(username=default_username)
            ...
            try:
                ...
                while 1:
                    ...
                    if not RE_VALID_USERNAME.match(username):
                        ...
                        continue
                    try:
                        IntranetUser.objects.get(username=username)
                    except IntranetUser.DoesNotExist:
                        break
                    else:
        ...
        IntranetUser.objects.create_superuser(username, email, password)
        if verbosity >= 1:
